...
{
    SDLTest_CommonState *state = (SDLTest_CommonState *)data;
...
static void SDLTest_PrintRenderer(SDL_Renderer *renderer)
{
    ...
    if (SDL_strcmp(name, "gpu") == 0) {
        SDL_GPUDevice *device = (SDL_GPUDevice *)SDL_GetPointerProperty(SDL_GetRendererProperties(renderer), SDL_PROP_RENDERER_GPU_DEVICE_POINTER, NULL);
